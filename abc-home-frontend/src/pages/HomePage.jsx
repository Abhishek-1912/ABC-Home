...
function HomePage() {
  ...
  useEffect(() => {
    Promise.all([
      fetchCategories(),
      fetchProducts({ size: 50 }),
    ])
      .then(([categoriesData, productsPage]) => {
        setCategories(categoriesData)

        const allProducts = productsPage.content.map(adaptProductSummary)
        const featured = allProducts.filter((p) => p.featured)

        // Fall back to showing whatever exists if nothing is marked featured yet
    ...
    <div className="min-h-screen bg-white text-gray-900">
        ...
        <div className="mx-auto grid min-h-[620px] max-w-7xl items-center px-4 sm:px-6 lg:grid-cols-2 lg:px-8">
          <div className="relative z-10 py-16 lg:py-24">
            ...
            <h1 className="max-w-xl text-5xl font-semibold leading-tight tracking-tight sm:text-6xl">
              Make your home
              <span className="block font-light">feel like you.</span>
            </h1>
      ...
      <section id="categories" className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
        <div className="mb-10 flex items-end justify-between">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.2em] text-gray-400">Explore</p>
            <h2 className="mt-2 text-3xl font-semibold tracking-tight sm:text-4xl">Shop by category</h2>
          </div>

          <Link to="/products" className="hidden items-center gap-2 text-sm font-medium sm:flex">
            View all
            <ArrowRight size={16} />
          </Link>
        </div>
        ...
        {loading ? (
          ...
          <p className="text-gray-400">No categories yet — add some from the admin panel.</p>
        ) : (
      ...
      <section id="featured" className="bg-gray-50">
        <div className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
          <div className="mb-10 flex items-end justify-between">
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.2em] text-gray-400">Curated for you</p>
              <h2 className="mt-2 text-3xl font-semibold tracking-tight sm:text-4xl">Best sellers</h2>
            </div>

            <Link to="/products?filter=best-sellers" className="hidden items-center gap-2 text-sm font-medium sm:flex">
              Shop all
              <ArrowRight size={16} />
            </Link>
          </div>
          ...
          {loading ? (
            ...
            <p className="text-gray-400">No products yet — add some from the admin panel.</p>
          ) : (
      ...
      <section className="bg-gray-900 text-white">
        <div className="mx-auto max-w-4xl px-4 py-20 text-center sm:px-6">
          <h2 className="text-3xl font-semibold sm:text-4xl">Make your space better.</h2>

          <p className="mx-auto mt-4 max-w-xl text-gray-400">
            Sign up for new arrivals, exclusive offers and inspiration for your home.
          </p>
